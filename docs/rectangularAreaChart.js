...
/* Data must be a an array of json objects formatted: [{value: 123, label: "Category 1", valuePrefix: "Some Prefix ", valueSuffix: " things"}, {value: 23, label: "Category 2", valuePrefix: "Some Prefix ", valueSuffix: " things"}] value and label are required. valuePrefix and valueSuffix are optional. */
function loadRectangularAreaChart(elementId, data, settings) {
  const dataSorter = function(a, b) {
    return a.value - b.value;
  };

  const valueFormatter = function(d, overrideValue) {
    let valueText = d.valuePrefix
      ? d.valuePrefix
      : "";
    valueText += overrideValue != null
      ? overrideValue
      : d.value;
    valueText += d.valueSuffix
      ? d.valueSuffix
      : "";
    return valueText;
  };
  ...
  if (settings == null)
    settings = rectangularAreaChartDefaultSettings();
  ...
  let dummyText = svg.append("text").attr("class", "rectangularAreaChartText").text("N");
  let textHeight = dummyText.node().getBBox().height;

  // Sort the data so that boxes are drawn in the right order.
  data.sort(dataSorter);
  data.reverse();
  let dataMax = Math.max(data[0].value, settings.maxValue);

  let width = parseInt(svg.style("width"));
  let height = parseInt(svg.style("height"));

  // Scales for the height and width of the boxes.
  let sizeScaleWidth = d3.scale.sqrt().range([0, width]).domain([0, dataMax]);
  let sizeScaleHeight = d3.scale.sqrt().range([0, height]).domain([0, dataMax]);
  ...
  let boxGroup = svg.selectAll("g").data(data).enter().append("g").attr("transform", function(d) {
    if (settings.animate) {
      let x = settings.expandFromLeft
        ? sizeScaleWidth(d.value) * -1
        : width;
      let y = settings.expandFromTop
        ? sizeScaleHeight(d.value) * -1
        : height;
      return "translate(" + x + "," + y + ")";
    } else {
      let x = settings.expandFromLeft
        ? 0
        : width - sizeScaleWidth(d.value);
      let y = settings.expandFromTop
        ? 0
        : height - sizeScaleHeight(d.value);
      return "translate(" + x + "," + y + ")";
    }
  })
  ...
  boxGroup.append("path").attr("id", function(d, i) {
    ...
    textY = settings.expandFromTop
      ? sizeScaleHeight(d.value) - settings.textPadding.bottom - textHeight / 4
      : textHeight + settings.textPadding.top;
    return line([
      {
        x: textX1,
        y: textY
  ...
  let labelPath = boxGroup.append("text").attr("class", "rectangularAreaChartText").style("fill", function(d) {
  ...
  if (settings.displayValueText) {
    ...
    boxGroup.append("path").attr("d", function(d) {
      ...
      if (settings.valueTextAlignDiagonal) {
        textY1 = settings.expandFromLeft
          ? sizeScaleHeight(d.value) - settings.valueTextPadding.left
          : settings.valueTextPadding.left;
        textY2 = settings.expandFromLeft
          ? settings.valueTextPadding.right
          : sizeScaleHeight(d.value) - settings.valueTextPadding.right;
      } else {
        if (settings.expandFromLeft) {
      ...
      textX = settings.expandFromLeft
        ? sizeScaleWidth(d.value) - settings.valueTextPadding.bottom - textHeight / 4
        : textHeight / 4 + settings.valueTextPadding.bottom;
      return line([
        {
          x: textX,
          y: textY1
    ...
    // Add the value text.
    let valuePath = boxGroup.append("text").attr("class", "rectangularAreaChartText").style("fill", function(d) {
      return settings.textColorScale(d.label);
    }).append("textPath").attr("startOffset", valueTextStartOffset).style("text-anchor", valueTextTextAnchor).attr("xlink:href", function(d, i) {
      return "#" + elementId + "VertPath" + i;
    });
    let valueText = valuePath.append("tspan"). // A tspan is necessary so that we can animate both the movement of the text and it's counting up from 0.
    text(function(d) {
      return settings.animate && settings.valueTextCountUp
        ? valueFormatter(d, 0)
        : valueFormatter(d);
    });
